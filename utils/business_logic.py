...
class AccountManager:
    ...
    @staticmethod
    def batch_rebind_expired_accounts() -> Dict[str, Any]:
        """
        一键换绑功能：将「已过期但被绑定」的账号批量换绑到新的可用账号

        业务逻辑：
        1. 查找所有「已过期但被绑定」的账号
        2. 为每个账号查找可用的新账号（未使用且生命周期未到期）
        3. 更新绑定关系：旧账号释放，新账号绑定，更新user_list
        """
        ...
        try:
            ...
            if not expired_but_bound_accounts:
                result['message'] = "没有需要换绑的账号"
                result['success'] = True
                return result

            rebind_details = []

            for old_account in expired_but_bound_accounts:
                try:
                    旧账号 = old_account['账号']
                    绑定学号 = old_account['绑定的学号']
                    套餐到期日 = old_account['绑定的套餐到期日']

                    if not 绑定学号:
                        result['failed_count'] += 1
                        result['details'].append(f"账号 {旧账号} 没有绑定学号，跳过")
                        continue
                    ...
                    if not available_accounts:
                        result['failed_count'] += 1
                        result['details'].append(f"学号 {绑定学号} 没有可用账号，换绑失败")
                        continue

                    新账号 = available_accounts[0]['账号']
                    ...
                    with db_manager.get_connection(enable_performance_mode=True) as conn:
                        ...
                        try:
                            ...
                            cursor.execute('''
                                UPDATE isp_accounts
                                SET 状态 = '已过期', 绑定的学号 = NULL, 绑定的套餐到期日 = NULL,
                            ...
                            cursor.execute('''
                                UPDATE user_list
                                SET 移动账号 = ?, 更新时间 = CURRENT_TIMESTAMP
                                WHERE 用户账号 = ?
                            ''', (新账号, 绑定学号))

                            cursor.execute("COMMIT")

                            result['rebind_count'] += 1
                    ...
                    result['failed_count'] += 1
                    result['details'].append(f"账号 {旧账号} 换绑失败: {e}")

            result['success'] = result['rebind_count'] > 0
            result['message'] = f"成功换绑 {result['rebind_count']} 个账号"
            ...
            # 添加成功的换绑详情到结果中
            if rebind_details:
                result['rebind_details'] = rebind_details

        except Exception as e:
            result['message'] = f"一键换绑失败: {e}"
